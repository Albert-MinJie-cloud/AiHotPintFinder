...
export async function collectForKeyword(keywordId: string): Promise<void> {
  const keyword = db.prepare('SELECT * FROM keywords WHERE id = ?').get(keywordId) as any;
  if (!keyword || !keyword.is_active) {
    console.log(`[Scraper] Keyword ${keywordId} is inactive or not found, skipping`);
    return;
  }
  ...
  try {
    ...
    console.log(`[Scraper] Found ${items.length} items for "${keyword.keyword}"`);

    // Verify authenticity via DeepSeek AI
    const verifiedItems = [];
    for (const item of items) {
      const verification = await verifyContentAuthenticity(
        keyword.keyword, item.title, item.content, item.source
      );
    ...
    const analyzedSpots = await analyzeHotSpots(
      keyword.keyword,
      verifiedItems.map(item => ({ title: item.title, content: item.content, source: item.source }))
    );
    ...
    for (const item of verifiedItems) {
      const analysis = analyzedSpots.find(
        a => a.title.toLowerCase().includes(item.title.toLowerCase().substring(0, 20))
      );
      ...
      const existing = db.prepare(
        'SELECT id FROM hot_spots WHERE keyword_id = ? AND title = ?'
      ...
      const kwExists = db.prepare(
        'SELECT id FROM keywords WHERE id = ? AND is_active = 1'
      ).get(keywordId);
      if (!kwExists) {
        console.log(`[Scraper] Keyword "${keyword.keyword}" was deleted or deactivated during collection, aborting`);
        break;
      }
      ...
      const notifId = db.transaction(() => {
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
          hotSpotId, keywordId, item.title, item.url || '', item.source,
          analysis?.summary || item.content.substring(0, 200), item.content,
          item.verification?.isAuthentic ? 1 : 0, item.verification?.score || 0.5,
          item.verification?.reason || '', hotScore, now,
          item.category || '', item.platform || '', item.relevance_score || 0,
        ...
        if (hotScore >= 60) {
          ...
          db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?)
          `).run(nid, hotSpotId, keywordId,
            `🔥 Hot spot detected: ${item.title} (score: ${hotScore})`, now);
          return nid;
        }
        return null;
      })();
      ...
      newHotSpots.push({
        id: hotSpotId, keyword: keyword.keyword, title: item.title,
        summary: analysis?.summary || item.content.substring(0, 200),
        hotScore, isAuthentic: item.verification?.isAuthentic,
        source: item.source, url: item.url, detectedAt: now,
      });

      if (notifId) {
        emitNotification({
          id: notifId, keyword: keyword.keyword, title: item.title,
          message: `🔥 Hot spot detected: ${item.title} (score: ${hotScore})`,
          isAuthentic: item.verification?.isAuthentic ?? true,
          hotScore, createdAt: now,
        });
      }
    }

    db.prepare('UPDATE keywords SET last_checked_at = ? WHERE id = ?').run(now, keywordId);
    ...
    if (newHotSpots.length > 0) {
      console.log(`[Scraper] Added ${newHotSpots.length} hot spots for "${keyword.keyword}"`);
    }
  } catch (error) {
    console.error(`[Scraper] Error collecting for "${keyword.keyword}":`, error);
  }
}

export async function collectAllKeywords(): Promise<void> {
  const keywords = db.prepare('SELECT * FROM keywords WHERE is_active = 1').all() as any[];
  console.log(`[Scraper] Starting collection for ${keywords.length} active keywords`);
